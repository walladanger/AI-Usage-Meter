...
export interface AppShellProps {
  ...
  renderRoute?: (route: ShellRoute) => ReactNode;
  requestedRoute?: ShellRoute;
  onMinimize?: () => void;
...
export function AppShell({
  ...
  const [activeRoute, setActiveRoute] = useState<ShellRoute>('overview');
  useEffect(() => { if (settings) setCollapsed(settings.settings.navigationCollapsed); }, [settings?.settings.navigationCollapsed]);
  useEffect(() => { if (requestedRoute) setActiveRoute(requestedRoute); }, [requestedRoute]);
      ...
      <main className="app-shell__workspace">
        <WindowManagerWorkspace navigationKey={activeRoute}>
          <div className="app-shell__workspace-content">
            {children ?? renderRoute?.(activeRoute) ?? <FoundationWelcome onNewWorkspace={onNewWorkspace} onOpenDataExplorer={onOpenDataExplorer} />}
          </div>
