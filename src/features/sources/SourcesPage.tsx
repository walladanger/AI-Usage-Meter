...
export function SourcesPage({ providers, onOpenSetup, onManualObservation }: SourcesPageProps) {
  ...
  return <section className="feature-page" aria-labelledby="sources-page-title">
    <header className="feature-page__hero"><span className="feature-page__eyebrow">Provider connectors</span><h1 id="sources-page-title">Sources</h1><p>Connect providers independently through the browser companion or keep a provider useful with manual observations.</p></header>
    <div className="source-list">{providers.map((provider) => <article className="feature-card source-card" key={provider.providerId}><div><strong>{provider.displayName}</strong><span>{provider.observation?.sourceType === 'manual' ? 'Manual entry' : provider.observation?.sourceType === 'browser_extension' ? 'Browser companion' : 'Not configured'}</span></div><span className="source-card__status" data-status={provider.status}>{provider.isFixture ? 'Preview' : provider.status.replace(/_/g, ' ')}</span></article>)}</div>
    <div className="feature-page__grid">
      <article className="feature-card"><h2><Puzzle size={17} /> Browser companion</h2><p>Pair the small extension to read only visible allowance values and reset times from signed-in provider pages.</p><button type="button" className="feature-button" onClick={onOpenSetup}>Open guided setup</button></article>
